...
def populateMatrices(seq1, seq2):
    ...
    scoringMatrix = createScoring(scoringMatrix, seq1, seq2)
...
def createScoring(scoringMatrix, seq1, seq2):
    for i in range(1,len(scoringMatrix)):
        for j in range(1,len(scoringMatrix[0])):
            scoringMatrix[i].append(max(c(seq1[j-1], seq2[i-1]) + scoringMatrix[i-1][j-1], scoringMatrix[i-1][j] - 2, scoringMatrix[i][j-1] - 2))
    return scoringMatrix

def maxValue(scoreD, scoreU, scoreL):
    pass
...
def initialiseBacktrack(backtrackMatrix, seq1, seq2):
    backtrackMatrix[0].append("END")
    for i in range(len(seq1)):
        backtrackMatrix[0].append("L")
    for i in range(len(seq2)):
        backtrackMatrix.append(["U"])
        backtrackMatrix[i+1].append([])
        backtrackMatrix[i+1].append([])
        backtrackMatrix[i+1].append([])
...
file1 = open('Sequence data/length3_A.txt', 'r')
seq1=file1.read()
file1.close()
file2 = open('Sequence data/length3_B.txt', 'r')
seq2=file2.read()
file2.close()
start = time.time()
